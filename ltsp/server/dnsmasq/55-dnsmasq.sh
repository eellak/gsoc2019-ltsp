...
dnsmasq_main() {
    test -f /etc/dnsmasq.d/ltsp-server-dnsmasq.conf &&
        die "Found old configuration: /etc/dnsmasq.d/ltsp-server-dnsmasq.conf
Aborting, to read the LTSP 5 upgrade guide please run: man ltsp upgrade"
...
dns_server() {
    ...
    dns_server=$(LANG=C.UTF-8 systemd-resolve --status |
...
# Generated by \`ltsp dnsmasq\`, see \`man ltsp dnsmasq\`
[Resolve]
DNSStubListener=no
EOF
